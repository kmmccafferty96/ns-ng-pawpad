...
import { ActivityStatusActions } from '../store/actions/activity-status.actions';
...
export class HttpStatusInterceptor implements HttpInterceptor {
    ...
    changeStatus(v: boolean, method: string): void {
        if (['POST', 'PUT', 'DELETE', 'PATCH'].indexOf(method) > -1) {
            ...
            this._store.dispatch(new ActivityStatusActions.SetActingStatus(this.actingCalls > 0));
        } else if (method === 'GET') {
            v ? this.loadingCalls++ : this.loadingCalls--;
            this._store.dispatch(new ActivityStatusActions.SetLoadingStatus(this.loadingCalls > 0));
        }
    }
}
